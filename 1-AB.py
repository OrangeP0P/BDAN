...
from ModelAblation import choose_net
from argparse import ArgumentParser
from DataLoader import read_model_data
from ModelAblation import handle_param
from ProgressBar import progress_bar
from TraningStrategyAblation import tranning_strategy
...
import math
import shutil
from TradeoffCalculation import cal_tradeoff

Seed_ACC = []  # 统计不同种子数时模型的最终准确率
Number = 1

for EX in range(Number, Number+1):

    l_s, l_t, l_cls, l_s_start, l_t_start, loss_speed_s, loss_speed_t, \
        Current_Datasets, Net_name, subject_list, total_task, data_size\
        = tranning_strategy(EX)  # 导入更新训练策略

    CSV_name = 'Ablation Experiment-' + str(EX) + '-ACC'  # 设置表格名称 每次开始时设置
    TXT_name = 'Ablation Experiment-' + str(EX) + '-OPTION'
    file_handle = open(TXT_name + '.txt', mode='w', encoding='utf-8')  # 生成笔记本：记录重要参数
    file_handle.write('消融实验' + str(EX) + '\n')
    file_handle.write('数据集' + Current_Datasets + '\n')
    file_handle.write('---------------[Net_Name:]' + str(Net_name) + '---------------\n')

    print('当前消融实验序号：' + str(EX))
    ...
    for seed in range(0, 1):  # 随机五个随机种子数
        ...
        Batch_size = 28
        Batch_size_list = [Batch_size] * 20  # batch_size
        ...
        Std_bridge = np.zeros([Terminal_Cross-Start_Cross+1, Terminal_Task-Start_Task+1,
                               Epoch, math.ceil(Batch_size/Batch_size_list[1])+2])
            ...
            for k in range(0, len(subject_list)):
                for g in range(0, len(subject_list)):
                    if subject_list[k] != subject_list[g] and Start_Task <= Task <= Terminal_Task:
                        ...
                        train_target_data, train_target_label, \
                        validation_target_data, validation_target_label, \
                        va_num \
                        ...
                        for epoch in range(args.epochs):
                            ...
                            la_s = 2 / (1 + math.exp(-10 * ((epoch - 50) / Epoch))) - 1
                            la_t = 2 / (1 + math.exp(-10 * ((epoch - 50) / Epoch))) - 1
                            ...
                            for i in range(0, num_iter):
                                ...
                                for key, net in net_dict.items():
                                    ...
                                    output, ele_loss, ele_loss_source, ele_loss_target, bridging_domain, std_bridge = net[0](
                                        train_source_data_1, train_target_data_1, bridging_domain)
                                    clf_loss = net[2](output, train_source_label_1.long())
                                    tr_loss = clf_loss + la_s * l_s * ele_loss_source + la_t * l_t * ele_loss_target
                            ...
                            # net[0].eval()#开启测试模式
                            for validation_data_1, validation_label_1 in validation_loader:
                                validation_data_1, validation_label_1 \
                                    = validation_data_1.cuda(), validation_label_1.cuda()

                                validation_data_1, validation_label_1 \
                                    = Variable(validation_data_1), Variable(validation_label_1)

                                va_pred, va_global_loss, va_intra_ele_loss, va_inter_ele_loss, bridging_domain, std_bridge = \
                                    net[0](validation_data_1, validation_data_1, bridging_domain.data)
                                va_clf_loss += net[2](va_pred, validation_label_1.long())
                                ...
                                _correct = pred.eq(validation_label_1.data.view_as(pred)).cpu().sum()  # 当前epoch中’分类正确样本数‘
                            ...
                            acc = correct / va_num
                            Std_bridge[Cross_Mission - Start_Cross + 1, Task - Start_Task, epoch, -1] = std_bridge  # 存入’准确率‘
                            ...
                            # 保存每个epoch的模型
                            # torch.save(net[0], 'Model/AblationEX/EX' + str(EX) + '/Task-' + str(Task) +
                            #            '/' + 'Cross-' + str(Cross_Mission + 1) + '_Task-' + str(Task) +
                        ...
                        # best_model_path = 'Model/AblationEX/EX' + str(EX) + '/Task-' + str(Task) + \
                        #                   '/' + 'Cross-' + str(Cross_Mission + 1) + '_Task-' + str(Task) + \
                        #                   '_epoch-' + str(acc_max_num) + '.pth'
                        #
                        # best_model_save_path = r'Model/AblationEX/EX' + str(EX) + '/Best_Model/' + \
                        #                        'Cross-' + str(Cross_Mission + 1) + '_' \
                        #                        'Task-' + str(Task) + '_epoch-' + str(acc_max_num) + '.pth'
                        #
                        # shutil.move(best_model_path, best_model_save_path)
        ...
        np.save('./Std_Bridge.npy', Std_bridge)  # 保存结果矩阵
