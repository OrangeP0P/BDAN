...
Examples:
    ...
    def __init__(self,
        ...
        self.channel_name = np.load(os.path.join(self.parent_dir,"config", f"{cap}_channel_name.npy"), allow_pickle=True)
        self.num_channels = self.channel_name.shape[0]
        # electrode_pos_raw或者source_signal_pos_raw代表电极的经纬度坐标，第一行theta 第二行phi。
        self.electrode_pos_raw = torch.tensor(np.load(os.path.join(self.parent_dir,"config", f"{cap}_channel_pos.npy"), allow_pickle=True)).type(
            torch.FloatTensor).to(self.device)
        self.source_signal_pos_raw = torch.tensor(np.load(os.path.join(self.parent_dir,"config", f"{cap}_channel_pos.npy"), allow_pickle=True)).type(
            torch.FloatTensor).to(self.device)
...
if __name__ == "__main__":
    # Create SVG object
    svg = SVG(cuda=True, sigma_rotate_theta=0.3, sigma_rotate_phi=0.1, cap='BCIIIIIV2A')
    # Input data
