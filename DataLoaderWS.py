...
def read_model_data(Basic_folder, Current_Datasets, subject_1, subject_2,
                    data_size, tr, va, train_source_list, train_target_list, validation_target_list):
    ...
    # 构建训练集、验证集、测试集 数据和标签
    train_source_data = source_data[train_target_list, :, :]
    _train_source_label = source_label[train_target_list, :]
    train_target_data = target_data[train_target_list, :]
    _train_target_label = target_label[train_target_list, :]
    validation_target_data = source_data[validation_target_list, :]
    _validation_target_label = source_label[validation_target_list, :]
