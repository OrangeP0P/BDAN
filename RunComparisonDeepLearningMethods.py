...
for EX in range(1, 4):
    ...
    Current_Datasets, Net_number, subject_list, total_task, data_size = tranning_strategy(EX)  # 导入更新训练策略
    ...
    for seed in range(0, 1):  # 随机五个随机种子数
        ...
        Batch_size = 40
        Batch_size_list = [Batch_size] * total_task  # batch_size
        ...
        Terminal_Task = total_task  # 设定结束 Task
                ...
                for g in range(0, len(subject_list)):
                    if subject_list[k] != subject_list[g] and Start_Task <= Task <= Terminal_Task:
                        ...
                        for epoch in range(args.epochs):
                            ...
                            # 保存每个epoch的模型
                            # torch.save(net[0], 'Model/ComparisonEX/EX' + str(EX) + '/Task-' + str(Task) +
                        ...
                        # best_model_path = r'Model/ComparisonEX/EX' + str(EX) + '/Task-' + str(Task) + \
                        #                '/' + 'Cross-' + str(Cross_Mission + 1) + \
                        #                '_epoch-' + str(acc_max_num) + '.pth'
                        # best_model_save_path = r'Model/ComparisonEX/EX' + str(EX) + '/Best_Model/' + \
